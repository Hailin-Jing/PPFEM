...
static FILE *eps_file;

static real xmin=INT_MAX;
static real xmax=INT_MIN;
static real ymin=INT_MAX;
static real ymax=INT_MIN;

/*==========================================================================*/
int start_eps(const char * eps_name)
{
    if((eps_file=fopen(eps_name,"w"))== nullptr)
    {
             ...
             int style, int width, int color, float le)
{
    ...
    if(style==1) fprintf(eps_file, "[%f] 0 sds ", double(le));
